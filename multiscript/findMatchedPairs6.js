        ...
        if (thirdToken != "" && thirdToken != firstToken) {
          for (let fourthPair of otherPairs) {
            ...
            if (fourthToken != "" && fourthToken != secondToken && fourthToken != firstToken) {
              for (let fifthPair of otherPairs) {
                let fifthToken = getPairsOtherToken(fifthPair, fourthToken);
                if (fifthToken != "" && fifthToken != thirdToken && fifthToken != secondToken && fifthToken != firstToken) {
                  for (let endPair of pivotPairs) {
                    let otherToken = getPairsOtherToken(endPair, PIVOT_TOKEN);
                    if (otherToken == fifthToken) {
                      const pathPairs = [
                      ...
                      if (!includedPaths.has(pathPairsJoined)) {
                        ...
                        matchObj.middlePools4.push(fifthPair["pairAddress"]);
...
console.log("\nHexagonal arbitrage paths saved to", MATCHED_PAIRS_OUTPUT_FILE);