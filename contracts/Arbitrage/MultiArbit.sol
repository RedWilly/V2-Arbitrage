...
contract MultiArbit {
    ...
    function multiSwap(
        ...
        require(pathLength >= 2 && pathLength <= 6, "Invalid path length");
        require(pools.length == pathLength, "Pools array length mismatch");
        require(amounts.length == pathLength * 2 + 1, "Amounts array length mismatch");
        ...
        for (uint256 i = 0; i < pathLength; i++) {
            ...
            address _to = i == pathLength - 1 ? msg.sender : pools[i + 1];
            IPancakePair(pools[i]).swap(amount0Out, amount1Out, _to, new bytes(0));
        }
    }

    function multiSwapBatch(
        uint256[][] memory amountsArr,
        address[][] memory poolsArr,
        address[] memory startTokens,
        uint256[] memory pathLengths
    ) external onlyOwner returns (bool[] memory) {
        require(amountsArr.length == poolsArr.length && poolsArr.length == startTokens.length && startTokens.length == pathLengths.length, "Array lengths mismatch");
        ...
        for (uint256 i = 0; i < size; i++) {
            (results[i], ) = address(this).delegatecall(
                abi.encodeWithSignature(
                    "multiSwap(uint256[],address[],address,uint256)",
                    amountsArr[i],
                    poolsArr[i],
                    startTokens[i],
                    pathLengths[i]
